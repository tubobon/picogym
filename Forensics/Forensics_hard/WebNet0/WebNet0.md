# WebNet0
## 解法
パケットキャプチャの問題で今回はプライベートキーがあった。\
```
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
```
これは、TLS暗号化の復号に使われるものらしく、復号できればTLS暗号化されていたhttpプロトコルを見ることができるらしい。\
つまりTLSプロトコルは、httpプロトコルを包んでいることになる。\
やり方としては、wiresharkで`編集`,`設定`,`Protocols`,`TLS`の項目にこの鍵を指定すると、復号したhttpプロトコルを見ることができた。\
復号したhttpパケット内にflagを発見した。`picoCTF{nongshim.shrimp.crackers}`

